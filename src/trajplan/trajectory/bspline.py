...
class UniformBSpline:
    ...
    def from_point_set(
        ...
        control_points: Matrix = np.asarray(
            np.linalg.solve(r_matrix, q_matrix.T @ b_matrix), dtype=np.float64
        )
    ...
    def evaluate_pva(self, t: float) -> Vector:
        ...
        Notes
        -----
        For t > duration, the spline is treated as a terminal hold:
        the position stays at the terminal point while velocity and
        acceleration are forced to zero. This avoids propagating a
        nonzero terminal derivative after the trajectory has expired,
        while still preserving the exact endpoint derivative when
        callers explicitly query t == duration.
        ...
        if t > self.duration:
            terminal_pva = np.asarray(
                self.evaluate_pva(self.duration),
                dtype=np.float64,
            ).reshape(-1)
