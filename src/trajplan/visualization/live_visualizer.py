...
class LiveVisualizer:
    ...
    def __init__(
        ...
        self._latest_target_snapshot: TargetVisualizationSnapshot | None = None
        self._target_history_positions: list[np.ndarray] = []
        self._target_artists: _TargetArtists | None = None
    ...
    def update(
        ...
        target_snapshot: TargetVisualizationSnapshot | None = None,
    ) -> None:
        ...
        self._ingest_target_snapshot(target_snapshot)
    ...
    def _ingest_target_snapshot(
        self,
        snapshot: TargetVisualizationSnapshot | None,
    ) -> None:
        if snapshot is None:
            return
        self._latest_target_snapshot = snapshot
        self._target_history_positions.append(
            np.asarray(snapshot.position, dtype=np.float64).reshape(3).copy()
        )
    ...
    def _ensure_target_artists(self) -> None:
        if self._target_artists is not None:
            return
        ...
        self._target_artists = _TargetArtists(point=point, history_line=history_line)
        self._refresh_legend()
    ...
    def _refresh_legend(self) -> None:
        ...
        if self._target_artists is not None:
            handles.append(self._target_artists.point)
    ...
    def _update_target_artists(self) -> None:
        if self._target_artists is None:
            return

        if self._latest_target_snapshot is None:
            self._target_artists.point.set_data_3d([], [], [])
            self._target_artists.history_line.set_data_3d([], [], [])
            return

        position = np.asarray(
            self._latest_target_snapshot.position,
            dtype=np.float64,
        ).reshape(3)
        self._target_artists.point.set_data_3d(
            [position[0]],
        ...
        if len(self._target_history_positions) == 0:
            self._target_artists.history_line.set_data_3d([], [], [])
            return

        history_points = np.vstack(self._target_history_positions)
        self._target_artists.history_line.set_data_3d(
            history_points[:, 0],
            history_points[:, 1],
            history_points[:, 2],
