...
from trajplan.planning.bspline_optimizer import (
...
class LocalPlanner:
    ...
    def __init__(
        ...
        max_v: float,
        max_a: float,
        optimizer: BsplineOptimizer | None = None,
    ) -> None:
        self.ctrl_pt_dist = float(ctrl_pt_dist)
        self.planning_horizon = float(planning_horizon)
        self.max_v = float(max_v)
        self.max_a = float(max_a)

        if self.ctrl_pt_dist <= 0.0:
            raise ValueError(f"Expected ctrl_pt_dist > 0, but got {self.ctrl_pt_dist}.")
        if self.planning_horizon <= 0.0:
            raise ValueError(
                f"Expected planning_horizon > 0, but got {self.planning_horizon}."
            )
        if self.max_v <= 0.0:
            raise ValueError(f"Expected max_v > 0, but got {self.max_v}.")
        if self.max_a <= 0.0:
            raise ValueError(f"Expected max_a > 0, but got {self.max_a}.")

        # Strictly follow simulator_v0.1 style.
        self.ts_check = self.ctrl_pt_dist / self.max_v * 1.2

        self.optimizer = (
            BsplineOptimizer(BsplineOptimizerConfig())
    ...
    def plan(
        ...
        # --------------------------------------------------------------
        optimized_bspline = self._iterative_collision_guided_optimization(
            grid_map=grid_map,
            initial_bspline=initial_bspline,
            local_target_pva=local_target_pva,
        )

        if optimized_bspline is None:
            return LocalPlanningResult(
                status="failure",
                trajectory=None,
        ...
        # --------------------------------------------------------------
        final_bspline = self._refine_bspline_feasibility(
            bspline=optimized_bspline,
        )
    ...
    def _generate_new_bspline(
        ...
        critical_distance = (self.max_v**2) / self.max_a
        if distance <= critical_distance:
            poly_duration = np.sqrt(distance / self.max_a)
        else:
            poly_duration = (
                distance - critical_distance
            ) / self.max_v + 2 * self.max_v / self.max_a
    ...
    def _extend_old_bspline(
        ...
        for t in np.arange(
            ...
            pt = old_bspline.evaluate_pva(float(t))[:3].reshape(-1, 1)
        ...
        if poly_time > self.ts_check:
            ...
            for t in np.arange(
                ...
                pt = onesegment_traj.evaluate_position(float(t)).reshape(-1, 1)
                segment_pts.append(pt)
        ...
        if len(segment_pts) < 2:
            segment_pts.append(
                np.asarray(local_target_pva[:3], dtype=np.float64).reshape(-1, 1)
            )
            pseudo_arc_length.append(
                np.linalg.norm(segment_pts[-1] - segment_pts[-2])
                + pseudo_arc_length[-1]
        ...
        if len(point_set) > (self.planning_horizon / self.ctrl_pt_dist) * 3:
            return None
    ...
    def _sample_polynomial_position_points(
        ...
        while True:
            ...
            for t in sample_times:
                pt = trajectory.evaluate_position(float(t)).reshape(-1, 1)
                point_set.append(pt)
                ...
                if max_dist >= self.ctrl_pt_dist * 1.5:
                    flag_too_far = True
                    break

            if len(point_set) == 0 or not np.allclose(
                point_set[-1].reshape(-1), trajectory.evaluate_position(duration)
            ):
                point_set.append(trajectory.evaluate_position(duration).reshape(-1, 1))
    ...
    def _iterative_collision_guided_optimization(
        ...
        lambda_dist = self.optimizer.config.lambda_collision
        pv_pair_info = [[] for _ in range(bspline.control_points.shape[0])]
        iter_cur = 0
        success_ctps_opt = False

        while iter_cur < self.optimizer.config.max_restart_num:
            ...
            result = self.optimizer.optimize(
                ...
                pv_pair_info=pv_pair_info,
                target_endpoint_pva=local_target_pva,
                lambda_dist=lambda_dist,
            )

            bspline = self._parse_optimization_result(result)
            if bspline is None:
                return None
    ...
    def _parse_optimization_result(
        result: BsplineOptimizationResult,
    ) -> UniformBSpline | None:
        if not result.success:
            return None
        return result.optimized_bspline

    # ------------------------------------------------------------------
    # Step 4: feasibility refine
    # ------------------------------------------------------------------

    def _refine_bspline_feasibility(
        self,
        bspline: UniformBSpline,
    ) -> UniformBSpline:
        feasible_flag, required_ratio = bspline.check_feasibility(
        ...
        return self._refine_feasibility(bspline=bspline, ratio=required_ratio)

    def _refine_feasibility(
        self,
        bspline: UniformBSpline,
        ratio: float,
    ) -> UniformBSpline:
