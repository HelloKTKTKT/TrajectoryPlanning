...
class LocalPlanner:
    ...
    def _generate_new_bspline(
        ...
        if tracking_reference is not None:
            effective_distance = max(
                distance,
                float(self.config.min_tracking_effective_distance),
            )
        poly_duration = self._estimate_travel_time_from_distance(effective_distance)
        if tracking_reference is not None:
            poly_duration = max(
                poly_duration,
                float(self.config.min_tracking_traj_duration),
            )
    ...
    def _extend_old_bspline_new_2(
        ...
        total_time = self._estimate_travel_time_from_distance(effective_length)
        if tracking_reference is not None:
            total_time = max(
                total_time,
                float(self.config.min_tracking_traj_duration),
