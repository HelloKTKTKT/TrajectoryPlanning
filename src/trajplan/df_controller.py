...
import numpy as np
import cvxopt as cvx  # pyright: ignore[reportMissingTypeStubs, reportUnknownVariableType]
from scipy.spatial.transform import Rotation  # pyright: ignore[reportMissingTypeStubs, reportUnknownVariableType]
...
class _QPSolution(TypedDict):
    status: str
    x: Any


def _to_cvx_matrix(value: Any) -> Any:
    return cast(Any, cvx.matrix(value))  # pyright: ignore[reportUnknownMemberType]
...
class DifferentialFlatnessController(CommonController):
    """
    ...
    def compute_control(
        ...
        qd_xyzw = Rotation.from_matrix(Rd).as_quat()
    ...
    def thrust_allocation_cvx(
        ...
        cvx.solvers.options["show_progress"] = False  # pyright: ignore[reportUnknownMemberType]
        solution = cast(
            _QPSolution,
            cvx.solvers.qp(  # pyright: ignore[reportUnknownMemberType, reportUnknownArgumentType]
                P=_to_cvx_matrix(self.P_cvx),
                q=_to_cvx_matrix(q_cvx),
                G=_to_cvx_matrix(self.G_cvx),
                h=_to_cvx_matrix(self.h_cvx),
            ),
        )
        ...
        rotor_thrust = np.array(solution["x"], dtype=np.float64).reshape(-1)
        return rotor_thrust
