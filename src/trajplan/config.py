...
class SwarmScenarioConfig:
    ...
    target: SwarmTargetScenario | None = None
...
def load_swarm_scenario_config(yaml_path: str | Path) -> SwarmScenarioConfig:
    ...
    if raw_target is not None:
        if not isinstance(raw_target, dict):
            raise ValueError("Expected target scenario entry to be a dict.")

        initial_position = np.asarray(
            raw_target["initial_position"],
            dtype=np.float64,
        ).reshape(-1)
        if initial_position.shape != (3,):
            raise ValueError(
                "Expected target.initial_position to have shape (3,), "
                f"but got {initial_position.shape}."
            )

        raw_waypoint_positions = raw_target.get("waypoint_positions", [])
        if not isinstance(raw_waypoint_positions, list):
            raise ValueError("Expected target.waypoint_positions to be a list.")

        waypoint_positions: list[np.ndarray] = []
        for waypoint_position in raw_waypoint_positions:
            waypoint = np.asarray(waypoint_position, dtype=np.float64).reshape(-1)
            if waypoint.shape != (3,):
                raise ValueError(
                    "Expected each target waypoint_position to have shape (3,), "
                    f"but got {waypoint.shape}."
                )
            waypoint_positions.append(waypoint)

        target = SwarmTargetScenario(
            initial_position=initial_position,
            waypoint_positions=waypoint_positions,
            speed=float(raw_target.get("speed", 0.0)),
            update_interval=float(raw_target.get("update_interval", 0.05)),
        )

    return SwarmScenarioConfig(agents=agents, target=target)
...
def build_bspline_optimizer_config(
    ...
    lambda_tracking = float(planning_cfg["bspline_optimizer"]["lambda_tracking"])
    ...
    tracking_sample_divisor = int(
        planning_cfg["bspline_optimizer"]["tracking_sample_divisor"]
    )
    config_build = BsplineOptimizerConfig(
        ...
        tracking_sample_divisor=tracking_sample_divisor,
    )
    return config_build
...
def build_planner_manager_config(
    ...
    tracking_enabled = bool(planning_cfg["planner"].get("tracking_enabled", False))
    tracking_history_size = int(planning_cfg["planner"].get("tracking_history_size", 10))
    tracking_update_interval = float(
        planning_cfg["planner"].get("tracking_update_interval", 0.25)
    )
    tracking_velocity_deadband = float(
        planning_cfg["planner"].get("tracking_velocity_deadband", 0.5)
    )
    tracking_prediction_time = float(
        planning_cfg["planner"].get("tracking_prediction_time", 5.0)
    )
    tracking_goal_update_thresh = float(
        planning_cfg["planner"].get("tracking_goal_update_thresh", 0.5)
    )
    tracking_replan_interval = float(
        planning_cfg["planner"].get("tracking_replan_interval", 0.5)
    )
    tracking_square_size = float(
        planning_cfg["planner"].get("tracking_square_size", 2.0)
    )
    tracking_height = float(planning_cfg["planner"].get("tracking_height", 1.5))
    tracking_goal_obstacle_margin = float(
        planning_cfg["planner"].get("tracking_goal_obstacle_margin", 0.2)
    )
    tracking_goal_search_step = float(
        planning_cfg["planner"].get("tracking_goal_search_step", 0.1)
    )
    tracking_goal_search_max_dist = float(
        planning_cfg["planner"].get("tracking_goal_search_max_dist", planning_horizon)
    )
    tracking_goal_map_margin = float(
        planning_cfg["planner"].get("tracking_goal_map_margin", 0.5)
    )
    tracking_square_scale_min = float(
        planning_cfg["planner"].get("tracking_square_scale_min", 0.8)
    )
    tracking_square_scale_max = float(
        planning_cfg["planner"].get("tracking_square_scale_max", 3.0)
    )
    tracking_square_scale_step = float(
        planning_cfg["planner"].get("tracking_square_scale_step", 0.25)
    )
    config_build = PlannerManagerConfig(
        ...
        tracking_velocity_deadband=tracking_velocity_deadband,
        tracking_prediction_time=tracking_prediction_time,
        tracking_goal_update_thresh=tracking_goal_update_thresh,
        tracking_replan_interval=tracking_replan_interval,
        tracking_square_size=tracking_square_size,
        tracking_height=tracking_height,
        tracking_goal_obstacle_margin=tracking_goal_obstacle_margin,
        tracking_goal_search_step=tracking_goal_search_step,
        tracking_goal_search_max_dist=tracking_goal_search_max_dist,
        tracking_goal_map_margin=tracking_goal_map_margin,
        tracking_square_scale_min=tracking_square_scale_min,
        tracking_square_scale_max=tracking_square_scale_max,
        tracking_square_scale_step=tracking_square_scale_step,
    )
    return config_build
...
def build_local_planner_config(
    ...
    min_tracking_traj_duration = float(
        planning_cfg["planner"].get("min_tracking_traj_duration", 1.0)
    )
    min_tracking_effective_distance = float(
        planning_cfg["planner"].get(
            "min_tracking_effective_distance",
            0.6 * float(planning_cfg["planner"]["planning_horizon"]),
        )
    )
    config_build = LocalPlannerConfig(
        ...
        min_tracking_traj_duration=min_tracking_traj_duration,
        min_tracking_effective_distance=min_tracking_effective_distance,
    )
    return config_build
