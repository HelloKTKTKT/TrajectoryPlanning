...
class CrazyflieAgentProcess(Process):
    ...
    Unlike the simulation agent process, this process constructs Crazyswarm,
    CrazyflieBackend, and MocapStateProvider inside ``run()`` so ROS/Crazyswarm
    objects do not need to cross process boundaries.
    """
    ...
    def _run_execution_loop(
        ...
        while not self.agent_stop_event.is_set():
            ...
            if isinstance(tick_output, PlannerTickOutput):
                agent.set_active_command(tick_output.command)
                print(
            ...
            if backend.emergency_triggered:
                print(
                    "[CrazyflieAgentProcess] emergency landing already triggered.",
                    flush=True,
                )
                break
            ...
            if agent.is_finished:
                print(
                    f"[CrazyflieAgentProcess] finished at t={now_time:.3f}",
                    flush=True,
                )
                break
